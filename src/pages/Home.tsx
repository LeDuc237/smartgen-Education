...
export default function Home() {
  ...
  {/* Content */}
  <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col h-full min-h-[80vh] justify-center">
 ...
 <motion.div
  ...
  transition={{ 
    duration: 0.8, 
    delay: 0.4,
    ease: [0.16, 1, 0.3, 1] // Courbe de transition plus naturelle
  }}
  className="text-center mb-8 sm:mb-12 space-y-4"
>
  {/* Titre d'accueil avec gradient animé */}
  <motion.h2 
  ...
  <motion.h1
    className="text-4xl sm:text-5xl md:text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-200 leading-tight"
  ...
  >
    {language === "en"
      ? "Find Your Perfect Home Teacher in Cameroon"
      : "Trouvez votre enseignant à domicile au Cameroun"}
  </motion.h1>

  {/* Ligne décorative animée */}
  <motion.div
    className="mx-auto h-1 bg-gradient-to-r from-blue-400/30 via-green-400/70 to-blue-400/30 rounded-full"
    initial={{ scaleX: 0 }}
    animate={{ scaleX: 1 }}
    transition={{
      delay: 0.8,
    ...
    style={{ originX: 0.5, width: '60%' }}
  />
</motion.div>

    {/* Stats */}
       <motion.div
      className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-8 px-2 sm:px-4 max-w-5xl mx-auto"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      {stats.map((stat, index) => (
        <motion.div
          key={stat.id}
          className="bg-white/25 backdrop-blur-sm rounded-lg p-3 sm:p-5 text-center border border-white/40 hover:border-white/60 shadow-lg hover:shadow-xl transition-all"
          initial={{ scale: 0.9, opacity: 0, y: 20 }}
          animate={{ scale: 1, opacity: 1, y: 0 }}
          transition={{ delay: 0.7 + index * 0.1 }}
          onClick={() => navigate(stat.path)}
        >
          <div className={`w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-2 sm:mb-3 rounded-full bg-gradient-to-r ${stat.gradient} flex items-center justify-center shadow-md`}>
            <stat.icon className="w-4 h-4 sm:w-6 sm:h-6 text-white" />
          </div>
          <p className="text-lg sm:text-2xl font-bold text-white drop-shadow-md mb-1">{stat.value}</p>
          <p className="text-[10px] sm:text-sm text-white/90 font-medium">{stat.title}</p>
        </motion.div>
      ))}
    </motion.div>
    ...
    {/* CTA Buttons */}
    <motion.div
      className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center w-full max-w-xl mx-auto"
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
    ...
    >
      <motion.button
        ...
        className="bg-gradient-to-r from-blue-600 to-green-500 hover:from-blue-700 hover:to-green-600 text-white px-6 py-3 rounded-full text-base sm:text-lg font-semibold shadow-xl hover:shadow-2xl flex items-center justify-center gap-2 w-full"
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.98 }}
      >
        <Target className="w-5 h-5" />
        {language === "en" ? "Discover Our Teachers" : "Découvrez nos enseignants"}
      </motion.button>

      <motion.button
        onClick={() => navigate("/teacher-register")}
        className="bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600 text-white px-6 py-3 rounded-full text-base sm:text-lg font-semibold shadow-xl hover:shadow-2xl flex items-center justify-center gap-2 w-full"
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.98 }}
      >
        <Users className="w-5 h-5" />
        {language === "en" ? "Join as Teacher" : "Rejoignez-nous"}
      </motion.button>
    </motion.div>
  </div>
</section>
                ...
                {topTeachers.map((teacher: Teacher, index) => (
                  ...
                  >
                    ...
                    <div className="relative h-40 sm:h-48 overflow-hidden">
                      ...
                      {/* Click to Zoom Indicator - always visible on mobile, hover on desktop */}
      ...
      <section className="section-padding bg-blue">
        <div className="container-custom">
          ...
          <div className="flex flex-col md:grid md:grid-cols-3 gap-6">
            ...
            {/* Teacher Portal */}
            <motion.div
              className="bg-gradient-to-br from-blue-50 to-green-50 rounded-xl shadow-lg p-5 border border-blue-100 flex flex-col justify-between h-full"
      ...
      <section className="py-12 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          ...
          <img
            src="/logofor.png"
            alt="FOROMAMED medical equipment"
